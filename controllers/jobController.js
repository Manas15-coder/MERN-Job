...
exports.createJobController = async (req, res) => {
    try {
        const { company, position, location, type, category, pay, description, url, user, applicants } = req.body
        if (!company || !position || !location || !type || !category || !pay || !description || !url) {
        ...
        const newJob = new jobModel({ company, position, location, type, category, pay, description, url, user: existingUser._id, applicants: applicants || [] });
